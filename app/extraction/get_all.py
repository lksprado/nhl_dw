...
def get_standings():
    ...
    data, _ = make_request(URL)
    save_json("standings", data, OUTPUT_DIR)
...
def get_team_roster_by_season():
    ...
    parameters_input = 'data/csv_data/processed/parameters_team_season.csv'
    df_parameter = pd.read_csv(parameters_input)
    urls = [URL.format(team_id=row.team_id, season_id=row.season_id) for row in df_parameter.itertuples()]
...
def get_club_stats_now(season_type=2):
    ...
    for team_id in unique_teams:
        url = URL.format(team_id=team_id, season_id=max_season_id, season_step=season_type)
        data = make_request(url)
        save_json(f"club_stats_{team_id}_{max_season_id}_{season_type}", data, OUTPUT_DIR)
        print(f"Data fetched for team {team_id} in season {max_season_id}")
        print(url)
...
def get_game_log(season_type=2):
    ...
    season_step = season_type
    URL = 'https://api-web.nhle.com/v1/player/{player_id}/game-log/{season_id}/{season_step}'
    ...
    df_filtered = df_parameter[~df_parameter.apply(lambda row: (str(row['player_id']), str(row['season_id'])) in existing_combinations, axis=1)]
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for row in df_filtered.itertuples():
            ...
            url = URL.format(player_id=player_id, season_id=season_id, season_step=season_step)
            futures.append(executor.submit(fetch_and_save_game_log, player_id, season_id, season_step, url, OUTPUT_DIR))
...
def get_player_info():
    ...
    df_filtered = df_parameter[~df_parameter.apply(lambda row: str(row['player_id']) in existing_combinations, axis=1)]
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for player_id in df_filtered:
            ...
            futures.append(executor.submit(fetch_and_save_player_info, player_id, url, OUTPUT_DIR))
