...
LOG_FILE = "app/raw_all_skaters_stats_ETL/raw_all_skaters__stats_log_log.log"
logger.remove()
logger.add(
...
def transform_skater_stats():
    ...
    OUTPUT_DIR = "data/csv_data/raw/raw_all_skaters_stats/staging"
...
def append_skaters_stats():
    ...
    INPUT_CSV_DIR = "data/csv_data/raw/raw_all_skaters_stats/staging"
...
def load_skaters_stats(file):
    ...
    try:
        update_table_with_sk(
            ...
            ["playerid", "seasonid", "gamesplayed", "teamabbrevs", "points"],
        )
        logger.info(f"✅ File {file} has been loaded in nhl_raw.raw_all_skaters_stats")
        return file
    except Exception as e:
...
def clear_staging_landing_loading(file):
    ...
    CSV_DIR = "data/csv_data/raw/raw_all_skaters_stats"
...
def run():
    ...
    try:
        ...
        appended_file = append_skaters_stats()

        if appended_file:  # Garante que um arquivo foi criado antes de tentar carregar
            loaded_file = load_skaters_stats(appended_file)
