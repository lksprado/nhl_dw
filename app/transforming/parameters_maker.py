...
def get_column(param_name: str, col: str, csv_filename: str):
    ...
    unique_values = set(
        map(str, df[col].to_list())
    )  # Convert values to string using map
    new_df = pl.DataFrame(
        {
...
def seasons_per_team():
    ...
    for file in files:
        df = pl.read_csv(file)
        team_id = (
            os.path.basename(file).replace("raw_team_season_", "").replace(".csv", "")
        )
        seasons = set(map(str, df["season"].to_list()))
        new_df = pl.DataFrame(
            {"team_id": [team_id] * len(seasons), "season_id": list(seasons)}
        )
